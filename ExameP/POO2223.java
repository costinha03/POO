...
public class POO2223 {
	...
	private static void alinea2(PrintStream out) {
		...
		try (Scanner input1 = new Scanner(new File("bookings.txt"), "UTF-8")) {
            ...
            while (input1.hasNext()) {
                ...
                try {
					...
					SpaceType spaceType = SpaceType.parse(fields[3]);
					String[] dim = fields[4].split("\\x");
					int[] dimensions = {Integer.parseInt(dim[1]),Integer.parseInt(dim[2])};
					input1.close();}
					catch (FileNotFoundException e) {
						System.out.println("Ficheiro não existente!");
					}
					Booking booking = new Booking(TaxID, startDate,duration);
		...
		// Deve devolver uma lista vazia (ou null) pois não existem espaços para carro
		// com a dimensão indicada (ou maiores)
		// This should return empty (or null) since there are no car spaces with the
		// required dimensions (or larger)
		out.println("Looking for car space with dimensions 300x600");
		List<CampingSpace> available = campingService.findAvailableCampingSpaces(SpaceType.CAR,
				LocalDate.of(2023, 6, 1), 60, new int[] { 300, 600 });
		...
		available = campingService.findAvailableCampingSpaces(SpaceType.CAR, LocalDate.of(2023, 6, 1), 60,
				new int[] { 300, 400 });
		if (available == null || available.isEmpty()) {
			out.println("** No options available for the type, dimensions, and dates requested.\n");
		} else {
		...
		// listar todos os espaços disponíveis a partir de 2023/12/01 por 60 dias, por
		// ordem decrescente de área total
		// list all spaces available from 2023/12/01 for 60 days, in decreasing order of
		// total area
